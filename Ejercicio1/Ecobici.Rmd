...
## Gráficas de afluencia por horarios
...
De la gráfica podemos notar que las horas pico de Retiro son a las 18:00,14:00,15:00 y 13:00 horas mientras que para arribos son las 14:00,18:00,15:00 y 19:00 horas.
Las horas pico parecen ser horarios de comida o salida de trabajo.

```{r}
grid.arrange(f1,g1)
```
Los días de lunes a Viernes tienen un comportamiento similar, mientras que para sabado y domingo, las horas pico de retiro son entre 12:00, 14:00 hrs mientras que para arribo son a las 13:00, 14:00 hrs 



## ¿En qué estaciones hay mayor afluencia de Retiros? 
...
Se nota una mayor afluencia de arribo por las tardes, recordando la gráfica de retiros, la estación 271 en particular tiene muchos retiros por las mañanas y  arribos por las tardes. La mayoría de estaciones consideradas son populares tanto en retiros como en arribos.

# 1.2. Perfiles de uso de las estaciones. 

**Empezaremos por un análisis exploratorio**
```{r}
...
### Estaciones más y menos concurridas retiros
```{r}
bu2 <- bike_use_retiro %>% 
    top_n(30) %>% 
    ggplot() +
...
### Estaciones más y menos concurridas arribos
```{r}
#Más concurridas
bu1<-bike_use_arribo %>% 
    top_n(30) %>% 
...
De la gráfica anterior resaltaría el hecho de que hay estaciones para las que el 50% de sus arribos son en fines de semana
...
```
A partir de 3 clusters se comienza a estabilizar la Suma De Cuadrados Interna
...
```
Estos 3 perfiles ya cuenta con carateristicas diferentes de otro grupo pero similares dentro del mismo grupo. Esta segmentación basicamente define perfiles en base a arribos y retiros, por lo que puede ser util para determinar la cantidad de bicicletas que deben tener en esas estaciones. Otra posible interesante agrupacion se puede hacer tomando en cuenta la variable de tiempos de uso, proporción de uso en fines de semana.
...
Recordando que:
El modelo de un proceso autorregresivo dice que en el momento $t$ el valor $Y_t$,consiste de una, $\delta$ (delta),más un coeficiente autorregresivo, $\phi$ (phi), por el valor del dato anterior ($Y_t$ - 1), más el ruido aleatorio, ɛtc.
La idea es tomar  $\phi$, medida de cambio, para verificar que tanto aumenta o disminuye la tendencia.

Primero haremos pruebas de existencia de tendencia

```{r c_28, message=TRUE, warning=TRUE}
...
Vamos a seleccionar solo las variables  para las que podemos rechazar la hipótesis nula de ausencia de tendencia a un nivel de confianza del 95%
